		...
		<div class="card-body">
			<form action="" id="manage_user">
					...
					<div class="col-md-6 border-right">
						...
						<div class="form-group">
							...
							<select name="type" id="type" class="custom-select custom-select-sm">
								<option value="3" <?php echo isset($type) && $type == 3 ? 'selected' : '' ?>>Empleado</option>
								<option value="2" <?php echo isset($type) && $type == 2 ? 'selected' : '' ?>>Jefe de Proyecto</option>
				...
				<div class="col-lg-12 text-right justify-content-center d-flex">
					...
					<button class="btn btn-danger" type="button" onclick="location.href = 'index.php?page=user_list'">Cancelar</button>
				</div>
			</form>
		</div>
	</div>
...
<script>
	$('[name="password"],[name="cpass"]').keyup(function(){
		...
		if(cpass == '' ||pass == ''){
			...
			if(cpass == pass){
				$('#pass_match').attr('data-status','1').html('<i class="text-success">Coincidencia de contraseñas.</i>')
			}else{
	...
	$('#manage_user').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					alert_toast('Datos guardados con exito.',"success");
					setTimeout(function(){
